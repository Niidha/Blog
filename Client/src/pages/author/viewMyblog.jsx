...
import AuthorLayout from "./authorlayout"; // Importing the AuthorLayout
...
function BlogsByAuthor() {
    ...
    const handlePublishToggle = async (blog) => {
        if (blog.reviewStatus !== "approved") {
            toast.error("This blog is not approved yet. Cannot publish.");
            return;
        }

        try {
            const response = await api.put(`/author/publish/${blog._id}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            setBlogs((prevBlogs) => 
                prevBlogs.map((b) => 
                    b._id === blog._id ? { ...b, published: updatedStatus } : b
                )
            );
                            ...
                            >
                                ...
                                <div className="p-4">
                                    ...
                                    <p className={`text-sm font-bold ${
                                        blog.reviewStatus === "approved" ? "text-green-500" : "text-yellow-500"
                                    }`}>
                                        Review Status: {blog.reviewStatus}
                                    </p>
                                    ...
                                    <label className="flex items-center cursor-pointer mt-3 relative">
                                        ...
                                        {blog.reviewStatus !== "approved" && (
                                            <span className="absolute -bottom-6 left-0 text-xs text-red-500 pt-3 pb-3">
